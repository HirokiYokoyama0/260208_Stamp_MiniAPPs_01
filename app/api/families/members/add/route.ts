...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[代理管理メンバー追加] リクエスト:', { userId, childName, ticketNumber });
    ...
    if (parentError || !parentProfile) {
      console.error('[代理管理メンバー追加] 親プロフィール取得エラー:', parentError);
      return NextResponse.json(
    ...
    if (parentProfile.family_role !== 'parent') {
      console.error('[代理管理メンバー追加] 権限エラー: family_role =', parentProfile.family_role);
      return NextResponse.json(
    ...
    if (!parentProfile.family_id) {
      console.error('[代理管理メンバー追加] 家族未登録エラー');
      return NextResponse.json(
    ...
    // 3. profiles テーブルに代理管理メンバーを追加
    const { data: newChild, error: insertError } = await supabase
      .from('profiles')
      .insert({
        id: virtualChildId,
        line_user_id: null, // 代理管理メンバーはLINEアカウントなし
    ...
    if (insertError) {
      console.error('[代理管理メンバー追加] 挿入エラー:', insertError);
      return NextResponse.json(
    ...
    console.log('[代理管理メンバー追加] 成功:', newChild);
    ...
    console.error('[代理管理メンバー追加] サーバーエラー:', error);
    return NextResponse.json(
