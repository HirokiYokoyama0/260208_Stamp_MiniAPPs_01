...
- **type:** `premium` … 優良患者様用
- **stamps:** `10` … 1回のスキャンで付与するスタンプ数（10個）
...
- **type:** `regular` … 通常患者様用
- **stamps:** `5` … 1回のスキャンで付与するスタンプ数（5個）

### 2.3 共通

- **重要**: フィールド名は `stamps` を使用すること（`points` ではない）
...
2. **デコード**
   読み取った文字列が上記JSON形式であればパースする。`type` と `stamps` を取得。不正な形式の場合はエラー表示し、再スキャンさせる。
...
- **入力:** LINEユーザーID（LIFFから渡す）、`type` または `stamps`、任意で `qr_code_id`。
- **処理:**
  1. `profiles` から `line_user_id`（または `id`）で該当患者を取得。
  2. 該当がなければ 404 または 400。
  3. `stamp_history` に1件INSERT: `user_id`, `visit_date = NOW()`, `stamp_number`（既存の最大値 + 今回の `stamps` など、既存ロジックに合わせる）, `amount = stamps`, `stamp_method = 'qr_scan'`, `qr_code_id`（あれば）。
...
3. LIFF側でAPIを呼び、成功したら「10個付与されました」等を表示。
4. 管理画面の患者一覧またはスタンプ履歴で、該当患者のスタンプが増えていることを確認する。
5. 同様に「通常患者様用」のQRで 5個 付与を確認する。
...
**⚠️ 重要な修正履歴:**
- 2026-02-22: 実装時に `points` → `stamps` に変更されました
- 2026-02-24: ドキュメントを実装に合わせて修正（`points` → `stamps`）

**最新の実装詳細:**
→ [Doc/82_QRコードスキャン機能_実装完了.md](82_QRコードスキャン機能_実装完了.md) を参照してください
