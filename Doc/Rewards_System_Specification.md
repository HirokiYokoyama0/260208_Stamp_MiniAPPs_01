...
INSERT INTO rewards (name, description, required_stamps, display_order) VALUES
  ('オリジナル歯ブラシセット', '当院推奨の歯ブラシ（ふつう/やわらかめから選択可）とフッ素配合歯磨き粉（30g）をセットでプレゼント。受付でお申し出ください。', 5, 1),
  ('フッ素塗布1回無料券', '次回来院時にフッ素塗布（通常¥1,100）を無料でご提供。虫歯予防に効果的です。有効期限: 交換日から6ヶ月間', 10, 2),
  ('歯のクリーニング50%OFF券', 'プロフェッショナルクリーニング（PMTC/通常¥5,500）を半額の¥2,750でご提供。歯石除去・着色汚れ除去を含む約30分コース。有効期限: 交換日から3ヶ月間', 15, 3),
  ('ホワイトニング1回30%OFF券', 'オフィスホワイトニング1回コース（通常¥16,500）を30%OFFの¥11,550でご提供。施術時間約60分。有効期限: 交換日から3ヶ月間', 20, 4);
```
...
4. API: スタンプ数が条件を満たしているか確認（12個 ≥ 10個 → OK）
5. API: **スタンプ数は減らさない**（12個のまま維持）
...
- スタンプは交換後も減らない
- 条件を満たせば何度でも交換可能
- 例: 20個貯まれば、5個の特典も10個の特典も15個の特典も20個の特典も全て交換可能
- 再来院のモチベーション向上を目的とした仕組み
