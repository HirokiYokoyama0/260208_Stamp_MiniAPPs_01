...
| カラー名 | 色コード | 用途 |
|---------|---------|------|
| kids-pink | #FFB5C8 | メインカラー、アクセント |
| kids-yellow | #FFE4A8 | 背景、ハイライト |
| kids-green | #A8DEBA | 成功メッセージ、達成時 |
| kids-blue | #A8D8E8 | ボタン、リンク |
| kids-purple | #C4B5E0 | 特典、ごほうび |

> 当初は蛍光色（#FF6B9D等）を使用していたが、目に優しいパステル系に変更済み（2026-04-11）
...
#### 1. KidsHome.tsx（会員証ページ）

**ファイルパス：** `components/(kids)/KidsHome.tsx`

**デザイン要件：**
- パステル系背景（薄紫〜水色グラデーション）
- ハブラーシカのイラストを大きく表示（`/images/はぶらしか正式.png`）
- 丸ゴシックフォント（font-kids = Zen Maru Gothic）
- 大きめのボタン（タップしやすく）
- 子供向けメッセージ（例：「はみがき がんばったね！」）

**スロットゲームへのアクセス:**
- 来院時に院内QRコードを読み取って解放する仕組み（詳細は [32_スロットゲーム仕様書](./32_子供モード_スロットゲーム.md) 参照）
- 未解放時: 「📷 QRをよんで ゲームかいほう！」ボタン → `liff.scanCodeV2()` でスキャン
- 解放後: 「🎰 ゲームで あそぶ！」ボタン表示
- アプリ内スキャン（`liff.scanCodeV2()`）は**キッズ画面専用**。大人画面では使用しない（iOS問題回避のため）

**Props：**
- `profileOverride?`: スマホなし子供の仮想メンバー表示用（内部でuseLiff、useViewModeも使う）
