...
### ユーザータイプ定義の見直し（2026-03-15）✅
- [x] 設計思想の修正
  - [x] 単身利用（family_role = NULL）を「デフォルト状態」として明確化
  - [x] 「親」「子」「単身」の3択ではなく、単身がベースであることを確立
- [x] UI変更
  - [x] 「個人で利用する」ボタン削除（不要な選択肢）
  - [x] 「現在、個人でご利用中です」説明文追加（状態の明示）
- [x] ロジック変更
  - [x] 初回リダイレクトを「知らせる」目的に変更
  - [x] localStorage flag `hasSeenFamilySetup` で初回表示管理
  - [x] 何もアクションしなければ単身利用継続
- [x] ドキュメント更新
  - [x] Doc 23: 用語定義に「単身ユーザー」追加、設計思想セクション追加、パターン0追加
  - [x] Doc 27: 管理ダッシュボード仕様書追加（reward_exchanges カラム不要結論）
  - [x] Doc 28: LIFF開発者向けドキュメント追加
  - [x] Doc 91: 実装履歴に記録
  - [x] Doc 90: TODO更新（本項目）
...
| 2026-02-08 | 最終アクセス日時（updated_at）と会員証番号（ticket_number）の表示機能を追加 |
...
| 2026-02-14 | 予約システム連携：会員証番号コピー機能、アポツール予約ボタン実装、会員証タブUIデザイン改善（メイン/サブボタンの洗練、余白調整） |
