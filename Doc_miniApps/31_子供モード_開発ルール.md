...
以下のリソースはアプリ全体で共有されています。**変更する場合は必ずメイン担当者に相談**してください。
...
1. feature ブランチをpush
2. Vercelダッシュボードでプレビュー環境のURLを確認
3. スマホなし子供画面の動作をテスト
4. 問題なければPull Request作成
5. メイン担当者がレビュー → mainにマージ → 本番デプロイ

### 環境変数

以下は既にVercelに設定済みです。**新しい環境変数が必要な場合はメイン担当者に依頼**してください。
...
Vercelでビルドが失敗した場合、ローカルで以下を実行して確認してください：
...
- **[Doc/30_子供モード仕様書.md](30_子供モード仕様書.md)**: 子ども用モードの全体仕様
