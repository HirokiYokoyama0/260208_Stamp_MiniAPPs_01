...
**来院時の付与:**
- 優良患者様: 1回の来院 = **15個**のスタンプ（1日1回まで）
- 通常患者様: 1回の来院 = **10個**のスタンプ（1日1回まで）
...
### 2.4 stamp_history.stamp_method
...
**値:**
```
"qr"                  - QR来院スタンプ（Premium/Regular）
"purchase_incentive"  - 購買インセンティブ
"qr_scan"             - QR来院スタンプ（旧実装、後方互換性）
...
- [x] `STAMP_AMOUNTS.PREMIUM_VISIT = 15` ✅ 2026-03-27 実装完了
- [x] `STAMP_AMOUNTS.REGULAR_VISIT = 10` ✅ 2026-03-27 実装完了
- [x] `STAMP_AMOUNTS.PURCHASE = 5` ✅ 2026-03-27 実装完了
- [x] `STAMP_AMOUNTS.SLOT_MIN = 3`
- [x] `STAMP_AMOUNTS.SLOT_MID = 5`
- [x] `STAMP_AMOUNTS.SLOT_MAX = 8`
- [x] `calculateStampDisplay()` は値をそのまま返す（割り算しない）
- [x] `formatStampCount()` はそのまま「○個」と表示
- [x] API実装で優良15個、通常10個、購買5個を付与 ✅ 2026-03-27 実装完了
- [x] 1日1回制限（購買インセンティブは除外） ✅ 2026-03-27 実装完了
- [x] スタッフ手動編集で入力値をそのまま保存
- [x] 画面表示で `stamp_count` をそのまま表示
- [x] 特典の必要スタンプ数を10倍に調整（5→50, 10→100, 15→150, 20→200）
...
| 日付 | 内容 |
|------|------|
| 2026-03-27 | 15スタンプ対応・購買インセンティブ追加（[Doc 54参照](./54_スタンプ仕様変更_実装完了報告.md)） |
