...
## ⚠️ 未使用・重複ファイル（2026-06-12 コード俯瞰調査）

> 参照確認により「どこからもimport/呼び出しされていない」ことを確認したもの。削除はリスク低だが運用中のため個別承認の上で。詳細は [124_コードベース棚卸し_実態調査レポート.md](124_コードベース棚卸し_実態調査レポート.md)。

| 種別 | パス | 状況 |
|------|------|------|
| コンポーネント | `components/dental/ToothDiagram.tsx`（v1） | ❌ 未使用。ケア記録は両ページとも `ToothDiagram_v2` を使用 |
| コンポーネント | `components/shared/KidsSlotButton.tsx` | ❌ 未使用。スタンプタブに統合済み（AppLayoutのimportも撤去済み） |
| APIルート | `app/api/rewards/route.ts`（GET） | ❌ フロント未使用。特典は `milestone_rewards` を直接SELECT |
| APIルート | `app/api/families/members/route.ts`（bare DELETE） | ❌ 未使用。`members/[memberId]` に置換済み |
| ルート直下 | `check_notes_location.ts` / `check_schema.ts` / `check_stamp_method.ts` / `investigate-kakiya-duplicate.ts` | 一回限り調査スクリプト。`scripts/` へ移動 or 削除候補 |
| デッドコード | `lib/analytics.ts`: `logPageView` / `logRewardExchange` / `logError` / `logChildScreenView` / `startSession` | 定義のみ・未使用（[60_イベントログ設計.md](60_イベントログ設計.md) §6） |

**重複・整理候補（コード）:**
- スタンプ付与エンドポイントが3系統（`/api/stamps`・`/api/stamps/scan`・`/api/stamps/auto`）でロジック重複（要集約・運用中につき段階的に）。
- ケア記録テーブルが2つ（実体 `patient_dental_records` / 空の `dental_records`）。
