# 家族機能 - 設計経緯と技術判断（アーカイブ資料）

> **⚠️ このドキュメントはアーカイブ資料です**
>
> - **最新の実装状況**: [22_Phase2_実装計画書.md](./22_Phase2_実装計画書.md)
> - **データベーススキーマ**: [05_Database_Schema.md](./05_Database_Schema.md)
>
> このドキュメントは Phase 1〜2 の設計検討過程を記録した資料です。
> **「なぜこの設計にしたか」** の経緯を知りたい場合にご参照ください。

---

## 📋 概要

現在の「1つのLINEアカウント = 1枚の会員証（1つのスタンプ数）」という1:1構造を、**家族単位でスタンプを共有できる仕組み**に拡張します。

**作成日:** 2026-02-16
**最終更新:** 2026-02-18
**ステータス:** ✅ Phase 1〜2 完了（アーカイブ資料）
...
COMMENT ON COLUMN stamp_history.amount IS '今回付与したポイント（通院=10点、スロット=3点〜8点）';
...
|-----------|----------------------|-------------|---------------------|------|
| 通常の通院 | +10点 | +1 | 累積 +10 | 会員証1枚につき付与 |
| スロット当選（最小） | +3点 | +0 | 累積 +3 | 通院時のみ発生するボーナス |
| スロット当選（中） | +5点 | +0 | 累積 +5 | 〃 |
| スロット当選（最大） | +8点 | +0 | 累積 +8 | 〃 |
...
### Phase 2: 家族テーブル・紐付け機能（1〜2日） - 🚧 未実装

```sql
-- 009_add_family_support.sql
...
- [03_機能仕様書.md](03_機能仕様書.md) - 既存機能の仕様
- [10_TODO.md](10_TODO.md) - タスク管理
...
| 2026-02-16 | 0.2 | Phase 1完了を反映、スロット付与値を「点」表記に統一（0.3〜0.8pt → 3点〜8点） |
