...
| テストケース | 手順 | 期待結果 |
|------------|------|---------|
| 子供の追加（親） | 名前・会員証番号を入力 → 保存 | line_user_id = NULL で子供が作成される |
| 子供の編集（親） | 子供の情報を編集 → 保存 | 子供の名前・会員証番号が更新される |
...
| 子供の画面表示 | キッズモード画面を開く | 子供の名前・会員証番号・スタンプが表示される |
...
- [21_家族ひもづけ機能_管理ダッシュボード仕様書.md](./archive/21_家族ひもづけ機能_管理ダッシュボード仕様書.md) - 管理側の仕様
- [03_Database_Schema.md](./03_Database_Schema.md) - データベーススキーマ
