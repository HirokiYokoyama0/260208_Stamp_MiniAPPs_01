...
**画面表示:**
- 家族あり: 「家族全体のスタンプ数」と表示（紫色）、個人スタンプ数も小さく併記
- 家族なし: 「現在のスタンプ数」と表示（通常色）
...
- スタンプ数は減らさない
- **注意**: スタンプ数チェックは個人ではなく、家族がいる場合は家族合算値を使用

### 2.5 ステータス管理

> ⚠️ **本表は旧定義（`available`/`expired` 導入前）。現行の正本は [114_availableステータス導入_実装完了レポート.md](114_availableステータス導入_実装完了レポート.md) §2.1、および交換フローの正本 [128_マイルストーン特典_交換フロー_あるべき姿.md](128_マイルストーン特典_交換フロー_あるべき姿.md)。**
> 現行ステータスは `available`（到達済・未交換）/ `pending`（交換申請済）/ `completed`（提供完了）/ `cancelled` / `expired` の5種。`available → pending → completed` が本来の遷移。
...
const handleReservation = async () => {
  if (displayTicketNumber === "未登録") {
    alert("会員証番号が登録されていません。受付にお声がけください。");
    return;
  }
...
- 詳細は [Doc/30_子供モード仕様書.md](30_子供モード仕様書.md) を参照
...
**詳細仕様:**
- [Doc/60_イベントログ設計.md](60_イベントログ設計.md) - 設計詳細
- [イベントログ仕様（管理ダッシュボード向け）](60_イベントログ設計.md) - 管理画面実装ガイド
...
患者の満足度や改善点を収集し、サービス品質向上に活用するアンケート機能。管理ダッシュボードで作成したアンケートが、LIFFアプリでモーダル表示される。

**目的:**
- 患者満足度の定量的な把握（星評価、NPS）
- 具体的な改善点の収集（自由記述）
- 定期的なフィードバック収集
- 回答者へのスタンプ報酬（30個）

**主な機能:**
1. アンケート表示制御（モーダルでの表示）
2. 複数質問タイプのサポート（星評価、NPS、自由記述）
3. スタンプ報酬の自動付与（30個）
4. 24時間再表示抑制機能（「後で」ボタン）
5. ターゲティング機能（特定ユーザーへの配信）

### 9.2 データベース設計

**surveysテーブル（アンケートマスター）:**
...
| questions | JSONB | 質問リスト（JSON配列） |
| is_active | BOOLEAN | 有効/無効フラグ |
| start_date | TIMESTAMPTZ | 配信開始日時 |
| end_date | TIMESTAMPTZ | 配信終了日時 |
| stamp_reward | INTEGER | 回答報酬（スタンプ数、デフォルト30個） |
| created_at | TIMESTAMPTZ | 作成日時 |
| updated_at | TIMESTAMPTZ | 更新日時 |
...
[
  {
    "id": "q1",
    "type": "star",
    "question": "当院のサービスに満足していますか？",
    "required": true
  },
  {
    "id": "q2",
    "type": "nps",
    "question": "友人や家族に当院を勧める可能性は？",
    "required": true
  },
  {
    "id": "q3",
...
**質問タイプ:**
- `star`: 星評価（1〜5段階）
- `nps`: Net Promoter Score（0〜10段階）
- `text`: 自由記述

**survey_answersテーブル（回答データ）:**

| カラム名 | 型 | 説明 |
|---------|---|------|
| id | UUID | 主キー |
| survey_id | UUID | アンケートID（surveys.id） |
| user_id | TEXT | 回答者ID（profiles.id） |
| answers | JSONB | 回答データ（JSON） |
...
**survey_targetsテーブル（ターゲット管理）:**

| カラム名 | 型 | 説明 |
|---------|---|------|
| id | UUID | 主キー |
| survey_id | UUID | アンケートID（surveys.id） |
| user_id | TEXT | 対象ユーザーID（profiles.id） |
| status | TEXT | 'pending' / 'answered' / 'postponed' |
...
- `pending`: 未回答（表示待ち）
- `answered`: 回答済み
- `postponed`: 延期（「後で」ボタンで24時間非表示）

**インデックス:**
```sql
CREATE INDEX idx_survey_targets_user_survey
  ON survey_targets(user_id, survey_id);

CREATE INDEX idx_survey_targets_status
  ON survey_targets(status);

CREATE INDEX idx_survey_answers_survey_id
  ON survey_answers(survey_id);

CREATE INDEX idx_surveys_active_dates
  ON surveys(is_active, start_date, end_date);
```
...
ALTER TABLE survey_targets
  ADD CONSTRAINT fk_survey_targets_survey_id
  FOREIGN KEY (survey_id) REFERENCES surveys(id) ON DELETE CASCADE;
```
...
アクティブなアンケートをチェック。回答済み・24時間以内に「後で」を押したアンケートは除外。

クエリパラメータ:
```
GET /api/survey/check?userId=U3c2f1a651af...
```
...
```json
{
  ...
  "survey": {
    "id": "abc123-def456-...",
    "title": "サービス満足度調査",
    "description": "ご来院ありがとうございます。簡単なアンケートにご協力ください。",
    "questions": [
      {
        "id": "q1",
        "type": "star",
...
**チェックロジック:**
1. `surveys` テーブルで `is_active = true` かつ配信期間内のアンケートを検索
2. `survey_targets` で対象ユーザーを確認
   - レコードなし → 全ユーザー対象
   - レコードあり → `user_id` が一致するもののみ
3. すでに回答済み（`status = 'answered'`）を除外
4. 24時間以内に延期（`status = 'postponed'` かつ `last_shown_at > NOW() - INTERVAL '24 hours'`）を除外
5. 最もpriorityが高い（作成日時が新しい）アンケートを1件返す

**POST /api/survey/submit:**

アンケート回答を送信し、スタンプ報酬を付与。

リクエスト:
```json
{
  "surveyId": "abc123-def456-...",
  "userId": "U3c2f1a651af...",
  "answers": {
    "q1": 5,
    "q2": 9,
...
動作:
1. `survey_answers` テーブルに回答を保存
2. `survey_targets` の status を 'answered' に更新、answered_at を記録
3. スタンプ報酬を付与:
   - `profiles.stamp_count` を +30
   - `stamp_history` に記録（`stamp_method: 'survey_reward'`）
4. トリガーで `profiles.stamp_count` が自動更新

レスポンス（成功）:
```json
{
  "success": true,
  "message": "アンケートのご回答ありがとうございました",
  "stampReward": 30,
  "newStampCount": 180
}
```
...
動作:
1. `survey_targets` の status を 'postponed' に更新
2. `last_shown_at` を現在時刻に更新
3. 24時間後に再度表示可能になる
...
{
  "success": true,
  "message": "24時間後に再度表示します"
}
```

### 9.4 UI実装

**1. アンケートモーダル（SurveyModal.tsx）:**

モーダルコンポーネントで、AppLayout.tsx から呼び出される。

表示タイミング:
- アプリ起動時（AppLayout の useEffect）
- `GET /api/survey/check` でアンケートがある場合のみ表示

モーダル構成:
- ヘッダー: タイトル、説明文
- ボディ: 質問リスト（質問タイプに応じたコンポーネント）
- フッター: 「後で回答する」「送信」ボタン
...
| コンポーネント | 役割 |
|-------------|------|
| StarRating.tsx | 星評価（1〜5段階、★アイコン） |
| NPSScale.tsx | Net Promoter Score（0〜10段階、ボタン選択） |
| TextInput.tsx | 自由記述（textarea） |

**3. 回答完了画面（SurveyCompleted.tsx）:**

回答後に表示される画面。

表示内容:
- チェックアイコン + お礼メッセージ
- スタンプ報酬表示（「30個のスタンプを獲得しました！」）
- 自動でモーダルが閉じる（3秒後）

**4. AppLayout.tsx での統合:**
...
useEffect(() => {
  ...
  // アンケートチェック
  fetch(`/api/survey/check?userId=${profile.userId}`)
    .then((res) => res.json())
    .then((data) => {
...
```
ユーザーがアプリ起動
  → AppLayout.tsx の useEffect 実行
  → GET /api/survey/check (userId)
  → アクティブなアンケート検索
    - 配信期間内 & is_active = true
    - 未回答 or 24時間以上前に延期
  → アンケートがある場合:
    → SurveyModal を表示
  → アンケートがない場合:
...
ユーザーがアンケートに回答
  ...
  → POST /api/survey/submit
    - survey_answers に保存
    - survey_targets を 'answered' に更新
    - スタンプ報酬を付与（+30個）
    - stamp_history に記録
  → SurveyCompleted 画面を表示
    - お礼メッセージ
    - 「30個のスタンプを獲得しました！」
  → 3秒後に自動でモーダルが閉じる
```
...
ユーザーが「後で回答する」をタップ
  → POST /api/survey/postpone
    - survey_targets を 'postponed' に更新
    - last_shown_at を現在時刻に更新
  → モーダルが閉じる
  → 24時間後に再度表示可能になる
```
...
2. 新しいスタンプ数を計算:
   ```typescript
   const stampReward = 30; // STAMP_AMOUNTS.SURVEY_REWARD
   const newStampCount = currentStampCount + stampReward;
   ```
...
3. `stamp_history` に記録:
   ...
   await supabase.from("stamp_history").insert({
     ...
     amount: stampReward,
     qr_code_id: `SURVEY-${surveyId}`,
     notes: `アンケート回答報酬: ${surveyTitle}`,
   });
   ```

4. トリガーで `profiles.stamp_count` が自動更新される

**STAMP_AMOUNTS定数（lib/stamps.ts）:**
```typescript
...
**全ユーザー対象の場合:**
- `survey_targets` にレコードを作成しない
- `/api/survey/check` で `survey_targets` にレコードがない場合、全ユーザーが対象

**特定ユーザー対象の場合:**
- 管理ダッシュボードで対象ユーザーを選択
- `survey_targets` に対象ユーザー分のレコードを作成
- `/api/survey/check` で `user_id` が一致するもののみ表示

**一括ターゲット登録SQL例:**
```sql
-- スタンプ数100個以上のユーザーを対象
INSERT INTO survey_targets (survey_id, user_id, status)
SELECT
  'survey-uuid-here',
  id,
  'pending'
FROM profiles
WHERE stamp_count >= 100;
```

### 9.8 24時間再表示抑制機能

**設計方針:**
- 「後で回答する」ボタンを押した場合、24時間は同じアンケートを表示しない
- ユーザー体験を損なわないための配慮
...
if (targets?.status === "postponed") {
  const lastShownAt = new Date(targets.last_shown_at);
  const now = new Date();
  const hoursSinceLastShown = (now - lastShownAt) / (1000 * 60 * 60);
...
SELECT
  s.title AS アンケート名,
  COUNT(DISTINCT st.user_id) AS 対象者数,
  COUNT(DISTINCT sa.user_id) AS 回答者数,
  ROUND(
    100.0 * COUNT(DISTINCT sa.user_id) / COUNT(DISTINCT st.user_id),
    2
  ) AS 回答率
FROM surveys s
...
```sql
SELECT
  AVG((answers->>'q1')::INTEGER) AS 平均評価
FROM survey_answers
WHERE survey_id = 'survey-uuid-here'
  AND answers->>'q1' IS NOT NULL;
```
...
WITH nps_data AS (
  SELECT
    (answers->>'q2')::INTEGER AS score
  FROM survey_answers
  WHERE survey_id = 'survey-uuid-here'
    AND answers->>'q2' IS NOT NULL
)
SELECT
  ROUND(
    100.0 * (
...
```sql
SELECT
  p.display_name AS 回答者,
  sa.answers->>'q3' AS 改善点,
  sa.created_at AS 回答日時
FROM survey_answers sa
JOIN profiles p ON p.id = sa.user_id
WHERE sa.survey_id = 'survey-uuid-here'
  AND sa.answers->>'q3' IS NOT NULL
  AND sa.answers->>'q3' != ''
...
3. タイトル・説明文を入力
4. 質問を追加:
   - 質問タイプを選択（星評価/NPS/自由記述）
   - 質問文を入力
   - 必須/任意を設定
5. 配信期間を設定
6. スタンプ報酬を設定（デフォルト30個）
7. ターゲットを設定:
   - 全ユーザー
   - 特定ユーザー（手動選択 or 条件指定）
8. 「作成」ボタンをクリック
9. アンケートが有効化され、対象ユーザーに配信開始
```
...
3. 回答状況を確認:
   ...
   - NPSスコア
   - 自由記述の一覧
4. データをCSVエクスポート（オプション）
```

### 9.11 技術的な設計判断

**1. モーダル表示の選択:**
- **メリット**: ユーザーの注意を引きやすい、回答率が高い
- **デメリット**: 邪魔になる可能性
- **対策**: 「後で回答する」ボタン + 24時間再表示抑制

**2. スタンプ報酬の自動付与:**
- アンケート回答のインセンティブ
- 報酬額は surveys.stamp_reward で柔軟に設定可能
- デフォルト30個（来院3回分相当）

**3. ターゲティングの柔軟性:**
- 全ユーザー対象: `survey_targets` を作成しない（シンプル）
- 特定ユーザー対象: `survey_targets` にレコードを作成（細かい制御）

**4. 24時間再表示抑制:**
- LocalStorageではなくサーバー側で管理
- デバイスを変えても一貫性を保つ
- 複数デバイス利用時も正しく動作
...
**アクセス制御:**
- 回答送信時に `userId` が一致するかチェック
- 回答データは `user_id` で関連付け（他人の回答は見られない）
- 管理ダッシュボードは認証必須（Phase 7で実装予定）

**データ整合性:**
- 外部キー制約: `survey_answers.survey_id` → `surveys.id`
- 外部キー制約: `survey_targets.survey_id` → `surveys.id`
- ON DELETE CASCADE: アンケート削除時、回答・ターゲットも削除

**スタンプ報酬の不正防止:**
- 同じアンケートに複数回回答できない（`survey_targets.status` チェック）
- APIレイヤーでの二重チェック

### 9.13 将来の拡張案

- [ ] 質問タイプの追加（複数選択、画像選択など）
- [ ] 条件分岐（前の回答に応じて次の質問を変える）
- [ ] アンケート結果のダッシュボード表示（グラフ化）
- [ ] LINE通知連携（アンケート配信時に通知）
- [ ] A/Bテスト機能（複数バージョンのアンケートを配信）
- [ ] 定期配信機能（毎月1回自動配信）
...
| 症状 | 対処 |
|-----|------|
| アンケートが表示されない | surveys.is_active, start_date, end_date を確認 |
| 24時間後も表示されない | survey_targets.last_shown_at と status を確認 |
| スタンプが付与されない | stamp_history にレコードが作成されているか確認 |
| 回答が保存されない | survey_answers テーブルを確認、JSON構造をチェック |
| ターゲットユーザーに表示されない | survey_targets.user_id が正しいか確認 |
...
| 日付 | 内容 |
|------|------|
| 2026-03-07 | アンケート機能仕様を追加（9章） |
| 2026-03-01 | 2.3 特典交換時のスタンプ数判定ルール（家族合算値対応）を追加 |
