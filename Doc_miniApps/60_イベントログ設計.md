...
| `home_page_view` | 会員証ページ閲覧 | `{current_stamp_count: 50}` |
...
### 8.1 個人情報保護
- **user_id のみ記録**: 名前や会員証番号は event_logs に保存しない
- **分析時に JOIN**: 必要な時だけ profiles テーブルと結合
- **匿名化オプション**: 必要に応じて user_id をハッシュ化

### 8.2 データ保持期間
- **現在の設定**: 400日間（約13ヶ月）
- **当初の設計推奨**: 90日間だったが、運用判断で400日に拡張済み
...
> ※ 旧 `84_イベントログ仕様_管理ダッシュボード開発者向け.md` の内容を統合
...
| 画面 | 内容 | 優先度 |
|------|------|--------|
| リアルタイムログストリーム | Supabaseリアルタイムサブスクリプションで最新100件表示 | Phase 1 |
| 総合ダッシュボード | DAU推移グラフ、人気機能TOP5、予約クリック数 | Phase 1 |
| イベント分析画面 | フィルター・日付範囲指定・CSV出力・ページネーション | Phase 1 |
| ユーザー行動フロー | ユーザーID/会員証番号検索 → 時系列タイムライン表示 | Phase 2 |
| キャンペーン効果測定 | 流入元(source)絞込 → 開封率グラフ | Phase 2 |
| SQL実行画面 | SELECT文のみ実行可能（INSERT/UPDATE/DELETE禁止）・プリセットクエリ登録・CSV出力 | Phase 3 |
...
#### 総合ダッシュボード表示内容
- 本日のアクティブユーザー数（DAU）
- 直近7日間のDAU推移グラフ（Chart.js または Recharts）
...
**重要**: 管理ダッシュボードでは全ログにアクセスできるよう、**サービスロールキー**を使用してください。
...
- **大量データ**: 常にページネーション使用（`.range(0, pageSize - 1)`）
- **日付範囲**: 常に `.gte('created_at', ...)` `.lte('created_at', ...)` を指定
- **インデックス活用**: user_id、event_name、created_at にインデックス設定済み

### 12.8 トラブルシューティング

| 症状 | 確認事項 |
|------|---------|
| ログが表示されない | LIFFアプリでイベント発生しているか？ / Supabase Table Editorにデータあるか？ / サービスロールキー使用しているか？ |
| パフォーマンスが遅い | 日付範囲を絞る / インデックスを活用 / ページネーション使用 |

### 12.9 データ保持期間

> ※ セクション8.2と同じ内容です。詳細はそちらを参照してください。
...
**最終更新日**: 2026-03-06（84_イベントログ仕様を統合）
**ステータス**: 設計完了・実装待ち
