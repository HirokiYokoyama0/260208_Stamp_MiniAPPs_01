...
- 会員証番号（ticket_number）表示（未登録時は「未登録」）
...
## 2026-03-27: マイルストーン型特典システム実装

### データベースマイグレーション
- **021_milestone_rewards_migration.sql**: milestone_rewards テーブル作成、milestone_history テーブル作成
- **022_update_validity_months.sql**: 有効期限設定（歯ブラシ0ヶ月、POIC 5ヶ月、自費メニュー5ヶ月）
- **024_fix_reward_id_foreign_key.sql**: reward_exchanges.reward_id の外部キー制約削除（新旧両テーブル参照のため）
- reward_exchanges テーブルに4カラム追加:
  - `milestone_reached` INTEGER: 到達したマイルストーン（10, 50, 300...）
  - `is_milestone_based` BOOLEAN: 新仕様フラグ（true）
  - `valid_until` TIMESTAMPTZ: 有効期限
  - `is_first_time` BOOLEAN: 初回特典フラグ（POIC用）
- UNIQUE制約追加: (user_id, reward_id, milestone_reached) で重複防止

### マイルストーン判定ロジック
- **lib/milestones.ts** 作成:
  - `checkMilestones()`: スタンプ数変化からマイルストーンを判定
  - `getRewardTypeForMilestone()`: マイルストーンに応じた特典タイプを決定
  - `grantMilestoneReward()`: 特典自動付与、milestone_history に記録
  - 優先度ルール実装: premium_menu(3) > poic(2) > toothbrush(1)
  - 初回判定: POICの初回/2回目を自動判定
  - 有効期限計算: 歯ブラシ(当日限り)、POIC/自費(5ヶ月後)

### API修正
- **app/api/stamps/auto/route.ts**: マイルストーン判定を統合、到達時に自動付与
- **app/api/stamps/scan/route.ts**: マイルストーン判定を統合
- **app/api/rewards/exchange/route.ts**:
  - milestone パラメータ追加
  - milestone_reached に正しい値を保存（fullStamps ではなく milestone）
  - 同一マイルストーンの重複チェック
- **app/api/rewards/route.ts**: rewards → milestone_rewards から取得するように変更

### フロントエンド実装
- **components/(adult)/AdultRewardsPage.tsx**:
  - 500スタンプ単位でページング表示（calculateDisplayRange, generateMilestonesInRange）
  - 優先度ルール適用（getRewardPriority）
  - handleExchange に milestone パラメータ追加
  - 交換ボタンクリック時に reward.nextMilestone を渡す
  - React Key警告修正: `${reward.id}-${reward.nextMilestone}`
- **types/reward.ts**: ExchangeRewardRequest に milestone 追加
- **lib/rewards.ts**: exchangeReward に milestone パラメータ追加

### 特典マスター（milestone_rewards）
現在のデータ（3件）:
1. 歯ブラシ 1本（every_10、当日限り）
2. POIC殺菌剤（every_50、5ヶ月、初回は本体込）
3. 選べるメニュー割引（every_150_from_300、5ヶ月）

### 優先度ルール
- 50スタンプ到達 → POICのみ（歯ブラシはスキップ）
- 100スタンプ到達 → POICのみ（歯ブラシはスキップ）
- 300スタンプ到達 → 選べるメニュー割引のみ（歯ブラシ・POICはスキップ）

### 管理ダッシュボード向け情報
- **外部キー制約削除**: Supabaseの自動JOINが使えないため、手動JOINが必要
- **正しいクエリ方法**:
  1. reward_exchanges を取得（is_milestone_based = true）
  2. milestone_rewards を取得
  3. アプリケーション側で手動JOIN（reward_id をキーに）
- **必須カラム**: milestone_reached, is_milestone_based

### ドキュメント整備
- **Doc_miniApps/55_特典システム_マイルストーン型_実装完了.md**: シンプルな実装完了報告
- **Doc_miniApps/58_管理ダッシュボード開発者へ_交換履歴の仕様変更.md**:
  - 管理画面が交換履歴を認識できない原因分析（4つの原因を特定）
  - 正しいクエリ方法（手動JOIN）
- **Doc_miniApps/05_Database_Schema.md**: v1.7 に更新
  - milestone_rewards テーブル追加
  - milestone_history テーブル追加
  - reward_exchanges テーブルの新カラム追加
  - rewards テーブルが未使用であることを明記
- **Doc_miniApps/60_マイルストーン型特典_実装完了レポート.md**: 詳細な実装レポート
- **Doc_miniApps/70_実装完了_15スタンプ対応.md**: 15スタンプ対応の記録

### 検証スクリプト
- **scripts/test-dashboard-query.mjs**: 管理ダッシュボード用クエリテスト（4つのケースを検証）
- **scripts/check-reward-exchanges.mjs**: 交換履歴確認
- **scripts/check-milestone-rewards.mjs**: 特典マスター確認
- **scripts/delete-50stamp-exchange.mjs**: 50スタンプ交換履歴削除用
- **scripts/check-all-schema.ts**: 全テーブルスキーマ確認

### 旧仕様からの移行
- **rewards テーブル**: データなし（未使用、新仕様では milestone_rewards を使用）
- **reward_exchanges**: 新旧両対応（is_milestone_based で区別）
- 外部キー制約削除により、reward_id が rewards または milestone_rewards の両方を参照可能
...
```typescript
const TABS = [
  { href: "/", label: "会員証", icon: CreditCard, kidsHref: undefined },
  // ... 他のタブも同様
...
## 2026-03-15: ユーザータイプ定義の見直し - 単身利用をデフォルトとして明確化

### 背景・問題

家族機能実装後、設定画面で「親」「子」「単身」の3つから選択する設計になっていたが、これは誤った設計思想:

- ❌ **誤解**: ユーザーは「親」「子」「単身」の3つから選ぶ
- ✅ **正解**: ユーザーはデフォルトで単身利用中。家族機能が必要な場合のみ、家族作成/参加を選択

### 設計思想の修正

**単身利用（`family_role = NULL`）は「選択肢」ではなく「デフォルト状態」**

- 全ての新規ユーザーは最初から単身利用状態
- 家族機能が不要なユーザーは、永続的に `family_role = NULL` のまま利用可能
- 単身利用は「未完了」の状態ではなく、正常な利用状態

### 変更内容

#### 1. UI変更（[app/settings/page.tsx](app/settings/page.tsx)）
...
**追加**:
- 「現在、個人でご利用中です」説明文（状態の明示）
- 「家族でスタンプを共有したい場合は、以下から設定できます」補足
...
<div className="space-y-4">
  ...
  {/* 家族グループを作成 */}
  <button onClick={() => router.push('/family/create')}>...</button>

  {/* 家族に参加 */}
  <button onClick={() => router.push('/family/join')}>...</button>
</div>
```

#### 2. ロジック変更（[components/(adult)/AdultHome.tsx](components/(adult)/AdultHome.tsx)）

初回リダイレクトの目的を明確化:

- **目的**: 家族機能の存在を「知らせる」ため（強制的に選択させるためではない）
- **動作**: 何もアクションを起こさなければ、そのまま単身利用を継続
- **実装**: localStorage flag `hasSeenFamilySetup` で初回表示済みを記録
...
#### 3. ドキュメント更新（[Doc_miniApps/23_家族機能_ユーザータイプ分類.md](Doc_miniApps/23_家族機能_ユーザータイプ分類.md)）

**用語定義に「単身ユーザー」を追加**:

| 正式用語 | 定義 |
|---------|------|
| **単身ユーザー** | `family_role = NULL`, `family_id = NULL` の状態。**これがデフォルトの利用形態** |
...
- [Doc_miniApps/27_家族ひもづけ機能_管理ダッシュボード仕様書.md](Doc_miniApps/27_家族ひもづけ機能_管理ダッシュボード仕様書.md): reward_exchanges テーブルへのカラム追加は不要と結論
- [Doc_miniApps/28_家族機能_LIFF開発者向け.md](Doc_miniApps/28_家族機能_LIFF開発者向け.md): 家族機能の実装ガイド

### 変更ファイル一覧

| ファイル | 変更種別 | 内容 |
|---|---|---|
| `app/settings/page.tsx` | 修正 | 「個人で利用する」ボタン削除、説明文追加 |
| `components/(adult)/AdultHome.tsx` | 修正 | 初回リダイレクトロジック修正（localStorage flag 追加） |
| `Doc_miniApps/23_家族機能_ユーザータイプ分類.md` | 修正 | 用語定義・設計思想・パターン0・FAQ更新 |
| `Doc_miniApps/27_家族ひもづけ機能_管理ダッシュボード仕様書.md` | **新規** | ダッシュボード仕様書 |
| `Doc_miniApps/28_家族機能_LIFF開発者向け.md` | **新規** | LIFF開発者向けドキュメント |

---

## 2026-03-28: ミニアプリ内QRスキャンイベントログの分離

### 問題

- ミニアプリ内の「来院スタンプを読み取る」ボタンのイベントログが管理ダッシュボードで確認できない
- カメラQRスキャンとミニアプリ内スキャンが同じイベント名で記録されていた
- どちらのスキャン方法が使用されたか判別できない

### 実装内容

#### 1. イベント名の分離（[lib/analytics.ts](lib/analytics.ts)）

`logStampScanSuccess` 関数に `scanMethod` パラメータを追加:
...
**イベント名**:
- カメラQR直接起動: `stamp_scan_success`
- ミニアプリ内スキャン: `stamp_scan_success_in_app`

#### 2. カメラQRスキャンの明示的指定（[app/api/stamps/auto/route.ts](app/api/stamps/auto/route.ts)）
...
#### 3. ミニアプリ内スキャンの明示的指定（[app/api/stamps/scan/route.ts](app/api/stamps/scan/route.ts)）
...
1. **スキャン方法の可視化**: カメラQRとミニアプリ内スキャンの利用率を比較
2. **問題の早期発見**: スキャン方法ごとのエラー率を分析
3. **A/Bテストが可能**: スキャン方法ごとのコンバージョン率測定

### ドキュメント

- [100_実装完了_ミニアプリ内QRスキャンイベントログ分離.md](Doc_miniApps/100_実装完了_ミニアプリ内QRスキャンイベントログ分離.md): 詳細仕様、SQLクエリ、テスト方法

### 変更ファイル一覧

| ファイル | 変更種別 | 内容 |
|---|---|---|
| `lib/analytics.ts` | 修正 | `logStampScanSuccess`に`scanMethod`パラメータ追加、イベント名分離 |
| `app/api/stamps/auto/route.ts` | 修正 | `scanMethod: 'camera'`を明示的に指定 |
| `app/api/stamps/scan/route.ts` | 修正 | `scanMethod: 'in_app'`を明示的に指定 |
| `Doc_miniApps/100_実装完了_ミニアプリ内QRスキャンイベントログ分離.md` | **新規** | 詳細仕様書 |

---

## 2026-03-28: 誕生月登録機能実装（お誕生日クーポン対応）

### 概要

管理ダッシュボード開発者の要望に基づき、ユーザーが誕生月を登録できる機能を実装。これにより、管理ダッシュボード側でお誕生日クーポンの自動配信が可能になる。

**コミット**: `6c62640` - feat: 誕生月登録機能を実装してお誕生日クーポン配信に対応

### 実装内容

#### 1. データベース（管理ダッシュボード側で実行済み）
...
ALTER TABLE profiles ADD CONSTRAINT birth_month_range CHECK (birth_month >= 1 AND birth_month <= 12);
```

- `birth_month`: INTEGER型、1-12の範囲制約、NULL許可
- 既存ユーザーのデータは NULL のまま（任意登録）
...
- 誕生月選択ドロップダウン追加（任意入力）
- インセンティブメッセージ: 「🎁 誕生月を登録すると、お誕生日クーポンが届きます！」
- `/api/users/setup-role` に `birthMonth` パラメータを送信
...
- Supabase経由で `profiles.birth_month` を直接更新
...
- [95_誕生日クーポン機能_技術的実現可能性と実装方針.md](Doc_miniApps/95_誕生日クーポン機能_技術的実現可能性と実装方針.md): 管理ダッシュボード開発者の仕様検討
- [96_誕生月クーポン機能_LIFFアプリ開発者へ.md](Doc_miniApps/96_誕生月クーポン機能_LIFFアプリ開発者へ.md): LIFFアプリ開発者向け仕様書
- [97_誕生月登録機能_実装完了レポート.md](Doc_miniApps/97_誕生月登録機能_実装完了レポート.md): **実装完了レポート**

### 変更ファイル一覧

| ファイル | 変更種別 | 内容 |
|---|---|---|
| `app/onboarding/page.tsx` | 修正 | 誕生月入力追加 |
| `app/settings/page.tsx` | 修正 | 誕生月表示・編集追加 |
| `components/(adult)/AdultHome.tsx` | 修正 | ポップアップロジック追加 |
| `components/shared/BirthMonthPromptModal.tsx` | **新規** | モーダルコンポーネント |
| `app/api/users/setup-role/route.ts` | 修正 | birthMonth パラメータ対応 |
| `supabase/025_add_birth_month_column.sql` | **新規** | DB マイグレーション（管理ダッシュボード側で実行済み） |
| `Doc_miniApps/95_誕生日クーポン機能_技術的実現可能性と実装方針.md` | **新規** | 仕様検討ドキュメント |
| `Doc_miniApps/96_誕生月クーポン機能_LIFFアプリ開発者へ.md` | **新規** | 実装仕様書 |
| `Doc_miniApps/97_誕生月登録機能_実装完了レポート.md` | **新規** | 実装完了レポート |

---


## 2026-04-03: セキュリティ強化（ミニマム対応）

### 実装の背景

**発見された脆弱性:**
- 現在のRLSポリシーの88.9%（24/27ポリシー）が `USING (true)` を使用
- ANON_KEYで全ユーザーのデータ取得が可能な状態
- 個人情報、医療記録、来院履歴が閲覧可能

**対策の方針:**
- ❌ 完全禁止（`USING (false)`）: LINEミニアプリが動作しなくなる
- ✅ **形式チェック（本実装）**: 既存コードは変更不要、全件取得攻撃のみ防ぐ
- 🟡 サーバーサイドAPI経由: 本格対応（2週間、中規模変更）

**重要な発見:**
- 既存のクライアントコードは全て `.eq("id", userId)` を使用
- つまり、クライアント側は既に正しく実装されている
- RLSポリシーに形式チェックを追加するだけで対応可能

### 実装内容

#### supabase/026_minimal_rls_hardening.sql
...
**例（components/(adult)/AdultHome.tsx）:**
```typescript
// 既存コード（変更不要）
const { data, error } = await supabase
  .from("profiles")
...
| 全件取得攻撃 | ❌ 可能 | ✅ 防御 |
| ランダムスキャン攻撃 | ❌ 可能 | ✅ 防御 |
| 他人のIDが分かっている場合 | ❌ 可能 | ⚠️ 依然として可能 |
| 総合評価 | ⭐ 最低 | ⭐⭐⭐ 中 |

**防げる攻撃:**
- ✅ `SELECT * FROM profiles` による全件取得
- ✅ ランダムなIDで総当たり攻撃

**防げない攻撃:**
- ⚠️ 正規のLINE User IDが分かっている場合の閲覧
- ⚠️ ブラウザDevToolsで `.eq()` の値を変更
...
const { data, error } = await supabase
  .from("profiles")
  .select("*");
console.log(error);  // RLSポリシーエラーが発生するはず
...
| 項目 | 値 |
|------|-----|
| 実装工数 | 1-2時間（SQLファイル作成のみ） |
| コード変更 | なし（既存コードは一切変更不要） |
| リスク | 🟢 最低（既存機能が壊れる心配なし） |
| ロールバック難易度 | 🟢 簡単（ポリシーを元に戻すだけ） |
| セキュリティ向上 | ⭐ → ⭐⭐⭐ |

### 次のステップ（今後の対応）

**Step 1: 今すぐ実施（本実装）**
- ✅ `026_minimal_rls_hardening.sql` を作成
- ⏳ Supabaseで実行（管理ダッシュボード開発者に依頼）
- ⏳ 動作確認

**Step 2: 中期対応（2週間以内、オプション）**
- サーバーサイドAPI経由への移行
- RLSポリシーをさらに厳格化（完全な他人データアクセス防止）

**Step 3: 長期対応（1ヶ月+、オプション）**
- Supabase Auth統合による完全なセキュリティ

### ドキュメント

**調査・分析:**
- [62B_セキュリティ対策_ミニマム実装案.md](Doc_miniApps/62B_セキュリティ対策_ミニマム実装案.md): 3つの対策案比較
- [63_管理ダッシュボード側への伝達事項.md](Doc_miniApps/63_管理ダッシュボード側への伝達事項.md): 管理ダッシュボードへの影響確認
- [64_セキュリティ調査レポート_2026-04-03.md](Doc_miniApps/64_セキュリティ調査レポート_2026-04-03.md): 詳細な脆弱性調査
- [64B_セキュリティ調査ツール_使い方.md](Doc_miniApps/64B_セキュリティ調査ツール_使い方.md): 調査スクリプト使い方
- [00_README_セキュリティ調査.md](Doc_miniApps/00_README_セキュリティ調査.md): セキュリティ調査の全体像

**実装方針（本対応）:**
- [65_真のミニマムセキュリティ対策.md](Doc_miniApps/65_真のミニマムセキュリティ対策.md): **形式チェックによる最小限対策**

### 作成ファイル一覧

| ファイル | 種別 | 内容 |
|---------|------|------|
| `supabase/026_minimal_rls_hardening.sql` | **新規** | RLSポリシー形式チェック追加 |
| `scripts/check-security-status.mjs` | **新規** | 実際のデータアクセステスト |
| `scripts/check-rls-policies.mjs` | **新規** | RLSポリシー動作テスト |
| `scripts/analyze-rls-in-sql.mjs` | **新規** | SQLファイル静的解析 |
| `Doc_miniApps/62B_セキュリティ対策_ミニマム実装案.md` | **新規** | 対策案比較 |
| `Doc_miniApps/63_管理ダッシュボード側への伝達事項.md` | **新規** | 管理側への影響確認 |
| `Doc_miniApps/64_セキュリティ調査レポート_2026-04-03.md` | **新規** | 脆弱性詳細レポート |
| `Doc_miniApps/64B_セキュリティ調査ツール_使い方.md` | **新規** | ツール使用方法 |
| `Doc_miniApps/00_README_セキュリティ調査.md` | **新規** | セキュリティ調査概要 |
| `Doc_miniApps/65_真のミニマムセキュリティ対策.md` | **新規** | 最小限対策方針 |

### 統計

- **SQLマイグレーション**: 1ファイル（279行）
- **調査スクリプト**: 3ファイル（計20.7KB）
- **ドキュメント**: 6ファイル（計60KB）
- **DROP POLICY**: 20ポリシー
...
## 2026-04-04: スタンプ減少時の特典削除未対応バグ修正（2件）

### 概要

スタッフがスタンプ数を手動で減らした際に発生する2つのバグを発見・修正:

1. **バグ1**: スタンプ減少時にマイルストーン特典が残り続ける
2. **バグ2**: スタンプ減少後のQRスキャンで古いスタンプ数が復活する
...
- スタッフ操作でスタンプ数を 200個 → 0個 に減らす
- マイルストーン特典（10, 20, 30...200）が `reward_exchanges` テーブルに残る
- ユーザーが実際には使えない特典を保持し続ける

#### 根本原因

[lib/milestones.ts:66-92](../lib/milestones.ts#L66-L92) の `checkMilestones()` 関数は増加のみ対応:
...
export function checkMilestones(
  ...
  for (let i = old10 + 1; i <= new10; i++) {  // ← 増加時のみ動作
...
スタンプ減少時は `old10 > new10` となり、ループが実行されない。

#### 修正内容

##### 1. `lib/milestones.ts` に `invalidateMilestoneRewards()` 関数を追加

[lib/milestones.ts:266-325](../lib/milestones.ts#L266-L325)
...
export async function invalidateMilestoneRewards(
  ...
  // 削除すべきマイルストーンを計算（例: 200→95 の場合 100,110...200）
  const old10 = Math.floor(oldStampCount / 10);
  const new10 = Math.floor(newStampCount / 10);
...
**動作:**
- 200→0: 10,20,30...200のマイルストーン特典を `status='cancelled'` に変更
- 200→95: 100,110...200のみ無効化、10-90は保持
- 使用済み(`status='used'`)の特典は無効化しない

##### 2. `app/api/stamps/manual/route.ts` に無効化処理を統合

[app/api/stamps/manual/route.ts:158-181](../app/api/stamps/manual/route.ts#L158-L181)
...
1. スタッフ操作: 200個 → 0個に減らす ✅
2. ユーザーが優良患者様用QRをスキャン（15個のはず）
3. **結果: 200個に復活** ❌

#### 根本原因

[supabase/008_update_stamp_triggers_for_notes.sql](../supabase/008_update_stamp_triggers_for_notes.sql) のトリガー関数:
...
BEGIN
  UPDATE profiles
  SET stamp_count = (
    SELECT MAX(stamp_number)  -- ← 全履歴の最大値を取得
    FROM stamp_history
    WHERE user_id = NEW.user_id
  ),
  updated_at = NOW()
...
**フロー:**
1. スタッフ操作: INSERT `stamp_number=0` → 履歴: [200(qr), 0(manual_admin)]
2. 直接 UPDATE `profiles.stamp_count = 0` (トリガーはスキップ) ✅
3. QRスキャン: INSERT `stamp_number=15` → 履歴: [200, 0, 15]
4. **トリガー発動: MAX(200,0,15) = 200** ❌

#### 修正内容

[app/api/stamps/manual/route.ts:113-155](../app/api/stamps/manual/route.ts#L113-L155)

**方針**: スタッフ操作を「新しい起点」として扱い、古い履歴を削除
...
const { data: historyData, error: historyError } = await supabase
  ...
  .insert({
    ...
    amount: changeAmount,
    stamp_number: newStampCount,  // これが新しい起点
    notes: `スタッフ操作: ${currentStampCount}個 → ${newStampCount}個 (${changeAmount > 0 ? '+' : ''}${changeAmount})`,
...
**動作（修正後）:**
1. スタッフ操作: DELETE 全履歴 → INSERT `stamp_number=0` → 履歴: [0]
2. 直接 UPDATE `profiles.stamp_count = 0` ✅
3. QRスキャン: INSERT `stamp_number=15` → 履歴: [0, 15]
4. **トリガー発動: MAX(0,15) = 15** ✅

**トレードオフ:**
- ❌ 古い履歴が失われる（意図的）
- ✅ スタッフ操作レコード（manual_admin）は監査証跡として残る
- ✅ スタンプ復活バグが完全に解消
...
| ファイル | 変更内容 |
|---------|---------|
| [lib/milestones.ts](../lib/milestones.ts) | `invalidateMilestoneRewards()` 関数追加（59行） |
| [app/api/stamps/manual/route.ts](../app/api/stamps/manual/route.ts) | 古い履歴削除処理追加（42行）<br>マイルストーン無効化処理追加（23行） |

#### ドキュメント

| ファイル | 内容 |
|---------|------|
| [Doc_miniApps/106_バグ報告_スタンプ減少時の特典削除未対応.md](106_バグ報告_スタンプ減少時の特典削除未対応.md) | 両バグの詳細レポート、根本原因、テストケース |
...
| テストケース | 期待される動作 | 結果 |
|-------------|---------------|------|
| 200個 → 0個 | 10,20,30...200 の特典が `status='cancelled'` | ✅ |
| 200個 → 95個 | 100,110...200 の特典が `status='cancelled'`<br>10,20...90 は保持 | ✅ |
| 既に `status='used'` の特典 | 無効化しない | ✅ |

#### バグ2: スタンプ復活防止

| テストケース | 期待される動作 | 結果 |
|-------------|---------------|------|
| スタッフ操作: 200 → 0 | `profiles.stamp_count = 0`<br>`stamp_history` が [0] のみ | ✅ |
| QRスキャン: +15 | `profiles.stamp_count = 15`<br>`stamp_history` が [0, 15] | ✅ |
| トリガー計算 | MAX(0, 15) = 15 | ✅ |
...
**Vercel自動デプロイ:**
```bash
$ git add lib/milestones.ts app/api/stamps/manual/route.ts Doc_miniApps/106_バグ報告_スタンプ減少時の特典削除未対応.md
$ git commit -m "fix: スタンプ減少時の特典削除未対応バグ修正（2件）"
$ git push origin main
```

**コミット履歴:**
- `fix: スタンプ減少時の特典削除未対応バグを修正`
- `fix: スタンプ復活バグを修正（スタッフ操作後のQRスキャン）`

---

### 統計

- **修正ファイル**: 2ファイル（計124行追加）
- **新規ドキュメント**: 1ファイル（[106_バグ報告_スタンプ減少時の特典削除未対応.md](106_バグ報告_スタンプ減少時の特典削除未対応.md)）
- **SQLマイグレーション**: なし（既存トリガー仕様を活用）
- **破壊的変更**: あり（スタッフ操作で古い履歴が削除される）
- **セキュリティ影響**: なし
